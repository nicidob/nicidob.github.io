...
[Benjamin Morris](https://fivethirtyeight.com/contributors/benjamin-morris/) remarked that *Elo is pretty much strictly dominated by having a running logistic regression over the same data*, with [extra clarification here](https://twitter.com/skepticalsports/status/1147225488273788929). The Elo formula for win probability between A and B (where A is the home team) is \\( \sigma(R_A - R_B + H) \\) where \\(H\\) is home court advantage. [Logistic Regression](https://en.wikipedia.org/wiki/Logistic_regression) computes probabilities using the formula \\( \sigma( \sum_i \alpha_i x_i + \beta) \\). If we create a variable for each team, add each match as a data example, give the home team a +1 and the away team a -1, then logistic regression can the optimal coefficients for our formula. Additionally, it discovers home court advantage automatically. By adding 1500 and multiplying the coefficients by 173, we can obtain Elo on the standard Elo scale.  The only real choice you have to make is how many games to use, more games lead to a smoother estimate. Here's an example with 10 games and 100 games.
<p align="center">
